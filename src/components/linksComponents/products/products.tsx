...
import { API_PRODUCT_CARDS, API_SEARCH } from "../../../constants/api";
...
const Products = ({ platform }: ProductsProps): JSX.Element => {
  ...
  const [urlTest, setUrlTest] = useState(API_SEARCH);
  ...
  const getResponse = async (url: string) => {
    ...
    setIsLoading(true);
    try {
      const res = await getApiCardResourse<Array<Card>>(url);
      setItems(res);
      setIsLoading(false);
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    ...
    if (age !== "all ages" && genres === "all genres") {
      getResponse(`${API_PRODUCT_CARDS + platform}&age=${age}&_sort=${sortByCriteria}&_order=${sortByType}`);
    }
    if (genres !== "all genres" && age === "all ages") {
  ...
  const onChange = (value: string, name?: string) => {
    if (name) setSortAndFilterProps({ ...sortAndFilterProps, [name]: value });
  };
